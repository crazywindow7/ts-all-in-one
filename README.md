...
# 기본 지식
- 메인 룰: **typescript는 최종적으로 javascript로 변환된다.** 순전한 typescript 코드를 돌릴 수 있는 것은 deno이나 대중화되지가 않았음. 브라우저, 노드같은 대중적인 런타임은 모두 js 파일을 실행한다.
...
## tsc의 두가지 역할
- 단순 타입 검사 (tsc --noEmit)
- typescript 코드를 javascript 코드로 바쒀줌
